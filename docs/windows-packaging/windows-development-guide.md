...
- 对中型和大型代码改动，以上命令必须由执行者本人实际运行，不能只在交付里建议别人去跑
- 对中型和大型代码改动，`build/test/lint` 只是最低门槛，不是完整验收
- 如果本轮触及保存、导出、导入、路由、平台桥或打包配置，不能只测 happy path
- 如果某一步没有自动化桌面测试，也必须留下明确的手动验收记录
- 构建产物已生成，不等于已经验证安装包或可执行文件在本机双击可启动
- 如果某个必要命令或必要手动场景本轮无法执行，必须在交付和 `DocsReview` 中明确写出阻塞原因与剩余风险，不能把本轮包装成“已完成 Windows 验收”
