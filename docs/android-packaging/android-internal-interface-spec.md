...
- 当前分支已把 `mode=new` 新建入口收口为带 `session` 标记的显式会话；生命周期恢复和方向切换后的重新进入不得误判成新的空白项目
- 当前分支已把草稿写回边界补到 `visibilitychange(hidden)`、`pagehide`、`beforeunload`，并要求显式保存后更新正式项目持久化指纹基线
- 当前分支已把持久化指纹提取到共享 helper；`TacticsEditor.tsx` 与 `useEditorState.ts` 必须共用同一套字段判断 dirty / saved baseline
- 当前分支已明确：与正式项目快照等价的项目草稿必须被清理，不能在“返回工作台 -> 继续编辑”时重新打开成脏状态
- 当前分支已明确：正式项目的保存基线只能在 `saveProjectState(...)` 成功后前移；保存失败时 UI 必须保留 dirty 状态并给出可见错误
...
- 任何持久化字段变化，包括 `playerPlacementTeam`、阵型与步骤上下文，都会重新参与 dirty 判断；不允许只靠局部 UI 状态推断“已保存”
- 当前 Slice 4 里，参考底图与球员头像在进入编辑器后仍会回写到前端项目状态；当前 Slice 5 已补上保存 / 恢复 / 生命周期边界实现，但这轮设备侧结论仍只到“模拟器验证通过”
- 当前 `Pixel_7` 手机模拟器与 `Pixel_Tablet_API_34` 平板模拟器记录可以支撑“模拟器验证通过”，但仍不能写成“真机完成”；真机覆盖仍留给 Slice 6
...
- 失败不允许破坏当前编辑状态
- 失败后用户应能立即重试
- 正式项目保存失败不得把 UI 误写成 `saved`，也不得偷偷前移正式项目持久化指纹基线
