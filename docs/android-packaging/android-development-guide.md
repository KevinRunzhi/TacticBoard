...
- 总路线、阶段门槛、范围边界，以 `android-packaging-plan.md` 为准
- 通过 / 不通过、完成 / 未完成，以 `android-acceptance-standard.md` 为准
- 实施目录、禁止做法、验证顺序、提交卫生，以本文件为准
- 切片顺序和单片退出条件，以 `android-phase1-slice-plan.md` 为准
- `slices/*.md` 负责把每个切片展开成可执行的开发闭环、接口检查和验收清单
- `DocsReview` 只记录证据和当轮结论，不反向覆盖前五类文档
...
### 5.2 生成目录、vendor 与临时补丁规则

Android 第一阶段经常会碰到三类“看起来像源码，实际上不是稳定源”的目录：
...
- 不把生成目录当成长期维护源码
- 不把 vendor 补丁目录当成无需说明的常规实现目录
- 不把 Gradle build 输出、APK / AAB、截图、logcat、录屏、UI dump 当成源码

如果某一轮为了验证技术路线，必须临时修改生成目录或 vendor 副本：

- 必须在当轮 `DocsReview` 中写清楚修改位置
- 必须写清楚对应的再生成路径或上游来源
- 在真正宣称切片退出前，必须把结论迁回稳定来源，或者明确说明该切片仍未关闭

不允许的做法：

- 只改 `src-tauri/gen/**`，然后把切片写成已完成
- 只改 `vendor/**`，不说明上游或再生成路径
- 把本地 APK、截图、日志当成长期可编辑交付物
...
- 每次提交前都要确认 `git status` 里只剩本轮真正要提交的代码、文档和测试
...
- 如果某一项只有自动化或推断性证据，要明确写成“已有自动化证据，但尚无设备侧硬证据”
...
- 一轮切片实现尽量包含“代码 / 配置 + 测试 + DocsReview 留痕”三个部分
- 不要把“代码实现”与“文档宣称切片完成”拆成长期漂移的两批提交
...
会稳定产出 unsigned universal release APK：

- `tactics-canvas-24/src-tauri/gen/android/app/build/outputs/apk/universal/release/app-universal-release-unsigned.apk`

如果目标是“可正式安装 / 分发”的 APK，本地还必须补一轮 release signing，而不是直接把 unsigned APK 发出去。

### 15.1 当前本地签名基线（2026-04-23）

- 当前已在本机生成 release keystore，并放在仓库外：
...
- keystore 和口令文件不得提交到仓库
- signed / aligned APK 仍然属于构建产物，不是文档或源码 source of truth
- 如果后续要保持同一应用升级能力，必须持续复用同一个 release keystore

### 15.2 推荐签名顺序

1. 先重跑 Android release 构建：

```bash
npm run tauri:android:build
```

2. 用 `zipalign` 对 unsigned APK 做对齐：

```bash
zipalign -p -f 4 app-universal-release-unsigned.apk app-universal-release-aligned.apk
```

3. 用 `apksigner` 做 release 签名：

```bash
apksigner sign --ks <release-keystore> --ks-key-alias tacticboard-release --out app-universal-release-signed.apk app-universal-release-aligned.apk
```

4. 用 `apksigner verify` 校验签名：

```bash
apksigner verify --verbose --print-certs app-universal-release-signed.apk
```

只有完成 `verify` 通过后，才能把这份 APK 当作当前阶段可安装 / 可分发的 release APK。

### 15.3 当前轮限制

- 2026-04-23 这一轮已经完成 signed APK 生成与签名校验
- 2026-04-23 复验 `adb install -r -g` 时，命中 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`
- 原因是设备里已存在一个不同证书签名的 `com.kevinrunzhi.tacticboard`
- 如果要在同一台设备上安装这份正式签名 APK，必须先卸载旧的 debug / 本地签名包
- 2026-04-23 后续在同一台手机上卸载旧包后，`adb install -g` 已成功，且 `adb shell am start -W -n com.kevinrunzhi.tacticboard/.MainActivity` 命中 `LaunchState: COLD`
- 因此，当前可以说：
  - `signed APK 已生成并完成签名校验`
  - `signed APK 已在真机完成一次干净安装与冷启动`
- 但不能把这一轮额外夸大成：
  - `signed APK 已在所有已有安装态设备上无损覆盖升级成功`
